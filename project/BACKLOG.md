...
| DRIVER-EARNINGS-001 | Implemented/CI-green; unmerged | [Backend PR23](https://github.com/SwaadFoodDelivery/food-delivery-backend/pull/23) adds `GET /driver/earnings` (sum of `orders.delivery_fee` over every order the driver actually delivered -- no separate commission/payout-rate concept exists in this schema); [frontend PR12](https://github.com/SwaadFoodDelivery/food-delivery-frontend/pull/12) adds an earnings section to the driver workspace, auto-refreshing on delivery completion. Fulfils the master prompt's Driver "permitted earnings/history" coverage check and the `driver_availability_earnings` flowchart (route path diverges from the flowchart's literal `/delivery/partner/earnings` to match this codebase's already-established `/driver/...` convention -- a documented decision, not an oversight). Backend verified against real local Postgres (disposable test DB, full migrations+seed), not just CI. No new frontend E2E coverage (disclosed gap). |
